...
export class LoggingInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    const isAllowed = LogUtil.isLogAllowed(request);
    const isEasypaisaRequest = LogUtil.isEasypaisaRequest(request);
    if (isEasypaisaRequest) LogUtil.maskEasypaisaCredentials(requestData);
    LogUtil.maskHeadersAuthorizationTokenIfExists(requestData);
    if (isAllowed) this._logRequest(requestData, ctx);
